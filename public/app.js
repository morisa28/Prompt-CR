...
async function loadMetadata() {
  ...
  setOptions(nodes.scenario, state.scenarios, (item) => item.id, (item) => item.title);
  setOptions(nodes.targetTool, state.targetTools, (item) => item, (item) => item);
  await loadQuestions();
}
...
function collectAnswers() {
  ...
  for (const question of state.questions) {
    if (question.answerType === "multi-choice") {
      const values = [...nodes.form.querySelectorAll(`input[name="${question.id}"]:checked`)].map((input) => input.value);
      if (values.length > 0) {
...
function renderResult(result) {
  ...
  renderBadge(result.generatedReview.passed);
...
function renderSummary(session) {
  ...
  nodes.requirementSummary.replaceChildren(...items.map(([label, value]) => {
...
function renderScore(node, score) {
  ...
  const color = score >= 85 ? "var(--good)" : score >= 70 ? "var(--accent)" : score >= 45 ? "var(--warn)" : "var(--danger)";
  node.style.background = `linear-gradient(135deg, ${color} 0%, ${color} 4px, #eef3f2 4px, #eef3f2 100%)`;
}

function renderBadge(passed) {
  nodes.reviewBadge.textContent = passed ? "自审通过" : "需修订";
  nodes.reviewBadge.className = `badge ${passed ? "good" : "warn"}`;
}
...
function renderMissingInfo(result) {
  ...
  const items = result.missingBlockingQuestions.length > 0
...
function renderLesson(result) {
  ...
  nodes.lessonRecord.append(list);
}
...
nodes.loadSample.addEventListener("click", applySample);
