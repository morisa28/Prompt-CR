...
当使用本 Skill Hub 生成 prompt 时，应把核心资源显式列出，尤其是在复杂或高风险任务中。

资源引用必须服务 prompt 质量：

- `@branch://...` 定义任务交付物和验收标准。
- `@template://...` 提供可复制的 prompt 骨架。
- `@checklist://...` 提供可判断的自检项。
- `@adapter://...` 约束目标工具的执行方式。
- `@safety://...` 收紧高风险领域边界。
- `@eval://...` 对照 expected 和 forbidden 检查 prompt。
- `@lesson://...` 注入已知失败模式或成功模式的修复经验。

不得为了显得完整而堆资源。每个资源都必须能改变 prompt 的清晰度、可执行性、可验证性、安全性或工具适配性。
